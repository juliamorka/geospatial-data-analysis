...
def get_missing_stations_info(
    ...
    **read_csv_kwargs
    ...
    """
    Retrieves missing stations and saves them to a JSON file
    in a form of {station_code: station_name}.

    Parameters:
        merged_stations_data (pd.DataFrame): Merged stations data DataFrame.
        stations_info_path (str): Path to the file containing
        ...
        **read_csv_kwargs: Additional keyword arguments to be passed to
           pandas read_csv function.
    """
    stations_info = pd.read_csv(stations_info_path, **read_csv_kwargs)
    stations_info.columns = stations_info_columns_labels
    merged = merged_stations_data.merge(
        stations_info, how="left", on=primary_key_col
    )
    ...
    with open("outputs/missing_stations_test.json", "w") as file:
        json.dump(
            dict(zip(missing.station_code, missing.station_name)),
            file,
            indent=4,
        )
...
def create_stations_voivodeship_mapping(
    ...
    """
    Creates a mapping between stations and voivodeships
    ...
    Parameters:
        ...
        voivodeship_boundaries_url (str, optional): URL to the file
            containing voivodeships boundaries data.
            Defaults to VOIVODESHIPS_URL.
    """
