...
from src.constants import DEFAULT_STATIONS_PLOT_PATH, VOIVODESHIPS_URL, DEFAULT_SPI_PRECIP_TS_PLOT_PATH
...
def plot_spi_and_total_precip_time_series(rolling_total_precip: pd.DataFrame, spi: pd.DataFrame,
                                          output_plot_path: str = DEFAULT_SPI_PRECIP_TS_PLOT_PATH) -> None:
    """
    Plot time series of Standardized Precipitation Index (SPI) and total precipitation for each station.

    Parameters:
        rolling_total_precip (pd.DataFrame): A DataFrame containing rolling total precipitation data
           for stations.
        spi (pd.DataFrame): A DataFrame containing Standardized Precipitation Index (SPI) data
          for stations.
    ...
    for ax, station_code in zip(axes.flat, unique_stations):
        ...
        rolling_total_precip_subset = rolling_total_precip[rolling_total_precip["station_code"] == station_code].copy()
        rolling_total_precip_subset["total_precip"] /= 500
        sns.lineplot(data=spi_subset, x="date", y="SPI", ax=ax, color="blue", label="SPI", legend=False)
        sns.lineplot(data=rolling_total_precip_subset, x="date", y="total_precip", ax=ax, color="red",
                     label="Total precipitation", legend=False)
        ax.set_title(f"Time series of SPI and total precipitation for station {station_code}")
