...
def calculate_gamma_parameters(data: pd.Series) -> tuple:
    ...
    Returns:
        tuple: A tuple containing the shape, location,
...
def calculate_gamma_cumulative_probability(
    value: float, params: tuple
) -> float:
    """
    Calculates the cumulative probability
    of a gamma distribution at a given value.

    Parameters:
        value (float): The value at which to
...
def get_spi(precipitation: pd.Series) -> list:
    ...
    for given precipitation data.

    Parameters:
        precipitation (pd.Series): A pandas Series
            containing precipitation data.

    Returns:
        list: A list containing SPI values
            for the input precipitation data.
    """
    gamma_params = calculate_gamma_parameters(precipitation)
    spi_values = [
        stats.norm.ppf(
            calculate_gamma_cumulative_probability(val, gamma_params)
        )
...
def calculate_stations_spi(
    stations_rolling_precip_sum: pd.DataFrame,
    save_to_csv: bool,
    output_csv_path: str = "outputs/spi.csv",
) -> pd.DataFrame:
    """
    Calculate Standardized Precipitation Index (SPI) for each station.

    Parameters:
        stations_rolling_precip_sum (pd.DataFrame): A DataFrame
           containing rolling precipitation sum data for stations.
        save_to_csv (bool): Flag indicating whether to
            save the result to a CSV file.
        output_csv_path (str, optional): Path to save the CSV file.
            Default is "outputs/spi.csv".

    Returns:
        pd.DataFrame: A copy of the input DataFrame
            with an additional column "SPI" containing
            the Standardized Precipitation Index values.
    """
    stations_rolling_precip_sum_copy = stations_rolling_precip_sum.copy()
    spi = (
        stations_rolling_precip_sum.groupby("station_code")["total_precip"]
        .apply(get_spi)
        .explode()
    )
