...
def transform_coords(merged: pd.DataFrame, coord_cols: list, crs: str = "EPSG:4326"):
    ...
    def parse_coordinates_to_degrees(encoded):
        return int(encoded[0:2]) + int(encoded[3:5]) / 60 + int(encoded[6:]) / 3600
...
def convert_date_info_to_timestamp(df: pd.DataFrame, date_cols: list) -> pd.DataFrame:
    """
    Converts specified date columns in the DataFrame to timestamps.

    Parameters:
        df (pd.DataFrame): The DataFrame containing the data.
        date_cols (list): A list of column names containing date information to be converted.

    Returns:
        pd.DataFrame: A new DataFrame with the specified date columns converted to timestamps.
    """
...
def drop_stations_below_threshold(df: pd.DataFrame, threshold: int) -> pd.DataFrame:
    """
    Drops stations from the DataFrame where the number of records is below the specified threshold.

    Parameters:
        df (pd.DataFrame): The DataFrame containing station data.
        threshold (int): The minimum number of records required for a station to be retained.

    Returns:
        pd.DataFrame: A DataFrame with stations dropped where the number of records is below the threshold.
    """
...
def fill_missing_dates(df: pd.DataFrame) -> pd.DataFrame:
    ...
    filling_info = (
        ...
        .set_index(["station_code"])[["station_name", "name", "lat", "lon", "geometry"]]
        .drop_duplicates()
    )
...
def interpolate_total_precip(df: pd.DataFrame) -> pd.DataFrame:
    """
    Interpolates missing values in the 'total_precip' column of the DataFrame using time-based interpolation.

    Parameters:
        df (pd.DataFrame): The DataFrame containing precipitation data.

    Returns:
        pd.DataFrame: A DataFrame with missing values in the 'total_precip' column interpolated.
    """
    interpolated = df.drop("precip_type", axis=1).reset_index().set_index("date")
